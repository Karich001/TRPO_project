...
const charJson = '/home/karich/projects_cpp/Game/humans.go/chars.json';
...
app.post('/img', upload.single('file'), async (req, res) => {
   try {
    let imageUrl;
      ...
      imageUrl = 'https://imgur.com/a/xH1ua7Q';
    } else {
      return res.status(400).send('Не предоставлен ни файл, ни URL');
    }
    ...
    const newChuvak = {
		...
		fs.writeFileSync('/home/karich/projects_cpp/Game/humans.go/chars.json', JSON.stringify(charData, null, 2), 'utf8');
...
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'Min.html'));
});
